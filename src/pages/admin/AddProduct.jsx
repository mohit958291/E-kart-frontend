...
function AddProduct() {
  ...
  const submitHandler = async(e)=>{
    ...
    try {
      setLoading(true)
      const res  = await axios.post(`https://e-kart-backend-1.onrender.com/api/v1/product/add`,formData,{
        headers:{
    ...
    <div className='pl-[350px] py-10 pr-20 mx-auto px-4 bg-gray-100'>
      <Card className='w-full my-20'>
        <CardHeader>
          ...
          <CardDescription>Enter Product details blow</CardDescription>
        </CardHeader>
        <CardContent>
          <div className='flex flex-col gap-2'>
            ...
            <div className="grid gap-2">
              <Label>Price</Label>
              <Input type='text' name='productPrice' value={productData.productPrice} onChange={handleChange} placeholder='Ex-Iphone' required/>
            </div>
          ...
          <CardFooter className="flex-col gap-2">
            <Button dispatch={loading} onClick={submitHandler} className="w-full mt-6 bg-pink-600 cursor-pointer" type='submit'>
            {
              loading ? <span className='flex gap-1'><Loader2 className='animate-spin'/>Please wait</span>:'Add Product'
            }
          </Button>
          </CardFooter>
        </CardContent>
